...
func main() {
	...
	components, err := bootstrap.Setup(ctx, "http-worker", bootstrap.WithoutDB())
	if err != nil {
	...
	components.Logger.Info("http-worker starting")
	...
	components.Logger.Info("http-worker started successfully")
	...
	components.Logger.Info("http-worker shutting down gracefully")
}
