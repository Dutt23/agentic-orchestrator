...
curl -s "$API_BASE/workflows/patch-test-1?materialize=false" | jq '{tag, kind, depth, patch_count}'
echo ""
curl -s "$API_BASE/workflows/patch-test-1?materialize=true" | jq '.workflow.nodes | length'
...
curl -s "$API_BASE/workflows/patch-test-2?materialize=false" | jq '{tag, kind, depth, patch_count}'
echo ""
curl -s "$API_BASE/workflows/patch-test-2?materialize=true" | jq '.workflow.nodes[0].config.timeout'
...
curl -s "$API_BASE/workflows/patch-test-3?materialize=false" | jq '{tag, kind, depth, patch_count}'
echo ""
curl -s "$API_BASE/workflows/patch-test-3?materialize=true" | jq '.workflow.nodes | length'
...
curl -s "$API_BASE/workflows/patch-base?materialize=true" | jq '.workflow.nodes[] | {id, type}'

echo -e "\n${BLUE}After Patch 1 (added notify_slack):${NC}"
curl -s "$API_BASE/workflows/patch-test-1?materialize=true" | jq '.workflow.nodes[] | {id, type}'

echo -e "\n${BLUE}After Patch 2 (updated timeouts):${NC}"
curl -s "$API_BASE/workflows/patch-test-2?materialize=true" | jq '.workflow.nodes[] | {id, timeout: .config.timeout}'

echo -e "\n${BLUE}After Patch 3 (removed enrich_data):${NC}"
curl -s "$API_BASE/workflows/patch-test-3?materialize=true" | jq '.workflow.nodes[] | {id, type}'
